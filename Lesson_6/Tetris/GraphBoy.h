...
uint16_t prevScore = 0xFFFF;
uint16_t prevLines = 0xFFFF;
uint8_t  prevLevel = 0xFF;
...
int fullLine();
void drawGameOver();
void printNum(int16_t x, int16_t y, uint8_t sz, uint16_t val, uint16_t &prev);
void updateStatus();
};

void GraphBoy::printNum(int16_t x, int16_t y, uint8_t sz, uint16_t val, uint16_t &prev) {
  if (val == prev) return;
  prev = val;
  tft.fillRect(x, y, FldLeft - 4, sz * 8, COL_PANEL);
...
void GraphBoy::updateStatus() {
  printNum(4, 26, 2, score, prevScore);              // ← розмір 2: вдвічі більший
  int16_t rx = FldLeft + Brd_W * 16 + 3;
  printNum(FldLeft + FldW + 4, 26, 2, level, (uint16_t&)prevLevel); // FldLeft + FldW + 4, 26 // rx, 22, 1,
  printNum(4, 90 , 2, lines, prevLines);  // rx, 54, 1,
}

void GraphBoy::drawGameOver() {
  int16_t gx = FldLeft + 4;
  tft.fillRect(gx, 116, Brd_W * 16 - 8, 64, COL_BG);
  tft.drawRect(gx, 116, Brd_W * 16 - 8, 64, COL_RED);
  tft.setTextColor(COL_RED);
  tft.setTextSize(2);
  tft.setCursor(gx + 14, 128); tft.print("GAME");
  tft.setCursor(gx + 18, 152); tft.print("OVER");
}

void GraphBoy::clearLine(byte num_line){
    for(int x = 0; x < Brd_W; x++) {
        display[x][num_line] = 0;
        wipePoint(x, num_line);
    }
}        
int GraphBoy::fullLine(){
    int lines = 0;
    bool res = false;
    for(int y = Brd_H - 1; y > -1; y--){
        byte count = 0;
        for(int x = 0; x < Brd_W; x++){
            if(display[x][y]){
                count++;
